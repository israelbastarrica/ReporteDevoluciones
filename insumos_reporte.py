...
function pedirProveedor(prov, secId) {{
  ...
  const tieneConsumo = r => r.ConsumidoDesdeIngreso > 0 || r.Consumido > 0;
      ...
      html+=`<tr data-cod="${{r.Codigo}}" class="${{rowCls}}">
        ...
        <td class="num" title="${{r.ConsumidoDesdeIngreso>=0?'Desde último ingreso':'Total histórico (sin ingreso registrado)'}}">
          ${{r.ConsumidoDesdeIngreso>0
            ...
            : (r.ConsumidoDesdeIngreso===0 && r.UltimoIngreso===HOY
...
function renderProveedores() {{
  ...
  cards.forEach(g => {{
    const cc = critCnt(g);
    const consCnt = g.pending.filter(r=>r.Consumido>0).length;
