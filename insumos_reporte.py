...
function pedirProveedor(prov, secId) {{
  const consEfectivo = r => r.ConsumidoDesdeIngreso>=0 ? r.ConsumidoDesdeIngreso : r.Consumido;
  const todos = (DATASETS[secId]||[]).filter(r =>
    r.Proveedor === prov &&
    !(shared.pedido_realizado||[]).includes(r.Codigo) &&
    !(shared.desuso||[]).includes(r.Codigo)
  );
  const conCons = todos.filter(r => consEfectivo(r) > 0);
  const sinCons = todos.filter(r => consEfectivo(r) === 0);
